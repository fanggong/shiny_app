...
base R中的字符串处理函数大部分是在调用R的内部函数，`stringr`是调用C函数。性能测试的结果在不同的环境，不同的数据集下有很大差距，解释其原理有些超出了我的知识范畴。所以在一个新的环境或数据集下，可以先使用`microbenchmark`包做一些简单测试，之后选择在当前环境下性能更优的处理方法。
