...
base R中的字符串处理函数大部分是在调用R的内部函数，`stringr`是调用C函数。先试着比较最简单的`str_length()`和`nchar()`。可以发现数据较大的情况下明显`stringr`性能领先于base R
```{r}
for (i in 1:7) {
  assign(paste0("str_", i), as.character(sample(999, 10^i, replace = TRUE)))
}
microbenchmark(
  str_length(str_1), nchar(str_1), str_length(str_2), nchar(str_2),
  str_length(str_3), nchar(str_3), str_length(str_4), nchar(str_4),
  str_length(str_5), nchar(str_5), str_length(str_6),nchar(str_6),
  times = 1
)
```
再尝试一下比较常用的正则匹配，数据较大的情况下`stringr`性能依旧领先于base R
```{r}
for (i in 1:6) {
  assign(
    paste0("stringr_", i), 
    paste0('str_replace_all(str_', i, ', "[0-3]", replacement = "X")')
  )
  assign(
    paste0("baser_", i), 
    paste0('gsub("[0-3]", "X", str_', i, ')')
  )
}
microbenchmark(
  eval(parse(text = stringr_1)), eval(parse(text = baser_1)),
  eval(parse(text = stringr_2)), eval(parse(text = baser_2)),
  eval(parse(text = stringr_3)), eval(parse(text = baser_3)),
  eval(parse(text = stringr_4)), eval(parse(text = baser_4)),
  eval(parse(text = stringr_5)), eval(parse(text = baser_5)),
  eval(parse(text = stringr_6)), eval(parse(text = baser_6)),
  times = 1
)
```
对于剩余函数用同样方法比较，结论基本相同