...
observeEvent(input$cor_cal, {
  ...
  if (method == "Pearson correlation coefficient") {
    output$cor_plot <- renderPlotly({
      validate(
        need(var_x_type == "numeric" && var_y_type == "numeric", "Variable X and Y must both be numeric variables")
      )
      plot_ly(
    ...
    output$cor_plot <- renderPlotly({
      validate(
        need(var_x_type == "ordered" && var_y_type == "ordered", "Variable X and Y must both be ordered variables")
      )
    ...
    output$cor_plot <- renderPlotly({
      validate(
        need(var_x_type %in% c("factor", "ordered") && var_y_type %in% c("factor", "ordered"), "Variable X and Y must both be factor/ordered variables")
      )
      tmp <- as.data.frame(table(x, y))
      names(tmp) <- c(var_x, var_y, "count")
      colors_dis <- colorRampPalette(c(BAR_COLOR, "#CCCCCC"))(length(levels(y)))
